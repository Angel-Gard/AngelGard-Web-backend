...
updateschedule: async function (data) {
    try {
        ...
        const query = `UPDATE scheduler SET scheduler_content = ?, scheduler_date = DATE(?) WHERE scheduler_id = ?`;
        await db.query(query, [scheduler_content, scheduler_date, scheduler_id]);
