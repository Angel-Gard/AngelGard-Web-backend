...
module.exports = {
    ...
    getAllBabiesByUserId: async function (user_id) {
        const sql = `SELECT * FROM baby WHERE user_id = ?`;
    ...
    deleteBaby: async function (user_id, baby_id) {
        const sql = `DELETE FROM baby WHERE user_id = ? AND baby_id = ?`;
        await db.query(sql, [user_id, baby_id]);
    }
};
