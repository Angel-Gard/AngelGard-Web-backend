...
module.exports = {
    selectComments: async function (req, res) {
        ...
        const sql = `SELECT user.user_image, user.user_nickname, comment_date, comment_content, comment.comment_id FROM comment 
        LEFT JOIN user ON comment.user_login_id=user.user_login_id WHERE board_id = ?`;
    ...
    createComment: async function (req, res) {
        const sql = `INSERT INTO comment (board_id, user_login_id, comment_content, comment_date ) VALUES(?,?,?,NOW())`;
        const [rows] = await db.query(sql, [Number(req.body.board_id), req.body.user_login_id, req.body.comment_content]);
