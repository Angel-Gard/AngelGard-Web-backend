        ...
        if (result && result.contents.length > 0) {
            result.contents = result.contents.map(item => {
                if (item.baby_board_image && !item.baby_board_image.startsWith('http')) {
                    item.baby_board_image = `http://localhost:3000/${item.baby_board_image}`; // 중복되는 'uploads/' 제거
                }
                return item;
            });
    ...
    try {
        ...
        if (result) {
            if (result.baby_board_image && !result.baby_board_image.startsWith('http')) {
                result.baby_board_image = `http://localhost:3000/${result.baby_board_image}`; // 중복되는 'uploads/' 제거
            }
            res.status(200).json(result);
        } else {
...
createbabyboard: async function (req, res, next) {
    try {
        const filePath = req.file ? `uploads/${req.file.filename}` : null; // 여기서 uploads/를 추가
